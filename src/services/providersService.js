...
async function updateProvider(id, { name, phone, specialty }) {
  const query = `UPDATE providers SET name = $1, phone = $2, specialty = $3 WHERE id = $4 RETURNING *`;
  const values = [name, phone, specialty, id];
  const result = await pool.query(query, values);
  return result.rows[0];
}
